...
from forms import ContactForm, AdminSettingsContactForm
...
def contact_form(request, form_class=ContactForm,
    ...
    **Template:**
    ...
    """
    #
    # Cuando no hay settings en la DB usamos el ContactForm por defecto
    # When there are not settings soterd in the DB, we use the default
    # ContactForm
    #
    if issubclass(form_class, AdminSettingsContactForm):
        if Settings.objects.count() > 0:
            form_settings = Settings.objects.all()[0]
            instructions = form_settings.instructions
        else:
